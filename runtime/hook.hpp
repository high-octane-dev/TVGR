...
struct ArgTranslator
{
    ...
    constexpr static std::enable_if_t<!std::is_pointer_v<T>, void> SetValue(PPCContext& ctx, uint8_t* base, size_t idx, T value) noexcept
    {
        ...
        {
            SetIntegerArgumentValue(ctx, base, idx, GLOBAL_MEMORY.map_virtual(value));
        }
        else
        {
    ...
    constexpr static std::enable_if_t<std::is_pointer_v<T>, void> SetValue(PPCContext& ctx, uint8_t* base, size_t idx, T value) noexcept
    {
        const auto v = GLOBAL_MEMORY.map_virtual((void*)value);
        if (!v)
        {
            return;
        }
...
T GuestToHostFunction(const TFunction& func, TArgs&&... argv)
{
    ...
    _translate_args_to_guest(newCtx, GLOBAL_MEMORY.base, args);

    SetPPCContext(newCtx);

    if constexpr (std::is_function_v<TFunction>)
        func(newCtx, GLOBAL_MEMORY.base);
    else
        GLOBAL_MEMORY.find_function(func)(newCtx, GLOBAL_MEMORY.base);
    ...
    {
        return reinterpret_cast<T>((uint64_t)GLOBAL_MEMORY.translate(newCtx.r3.u32));
    }
...
#define GuestFunctionHook(subroutine, function) \
    PPC_FUNC(subroutine) { HostToGuestFunction<function>(ctx, base); }

#define GuestFunctionStub(subroutine) \
    PPC_FUNC(subroutine) { }