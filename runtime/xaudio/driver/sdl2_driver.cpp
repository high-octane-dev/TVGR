...
constexpr auto _MAX_VOLUME = 1.0f;
...
static void AudioThread()
{
    ...
    while (!g_audioThreadShouldExit)
    {
        ...
        if ((queuedAudioSize / callbackAudioSize) <= MAX_LATENCY)
        {
            ...
            g_clientCallback(ctx.ppcContext, GLOBAL_MEMORY.base);
        }
...
void XAudioRegisterClient(PPCFunc* callback, uint32_t param) {
    auto* pClientParam = static_cast<uint32_t*>(GLOBAL_USER_HEAP.alloc(sizeof(param)));
    ByteSwapInplace(param);
    *pClientParam = param;
    g_clientCallbackParam = GLOBAL_MEMORY.map_virtual(pClientParam);
...
void XAudioSubmitFrame(void* samples)
{
    ...
    if (g_downMixToStereo)
    {
        ...
        for (size_t i = 0; i < XAUDIO_NUM_SAMPLES; i++)
        {
            ...
            audioFrames[i * 2 + 0] = (ch0 + ch2 * 0.75f + ch4) * _MAX_VOLUME;
            audioFrames[i * 2 + 1] = (ch1 + ch2 * 0.75f + ch5) * _MAX_VOLUME;
        }
    ...
    else
    {
        ...
        for (size_t i = 0; i < XAUDIO_NUM_SAMPLES; i++)
        {
            for (size_t j = 0; j < XAUDIO_NUM_CHANNELS; j++)
                audioFrames[i * XAUDIO_NUM_CHANNELS + j] = floatSamples[j * XAUDIO_NUM_SAMPLES + i] * _MAX_VOLUME;
        }
...
void XAudioConfigValueChangedCallback(IConfigDef* configDef)
{
    ...
    if (configDef == reinterpret_cast<IConfigDef*>(&IS_SURROUND)) {
